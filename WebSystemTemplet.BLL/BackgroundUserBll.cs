...
namespace WebSystemTemplet.BLL
{
    public static class BackgroundUserBll
    {
        ...
        /// 登陆检测
        /// </summary>
        //public static Model.UserInfo Login(string UseName, string password, string ipAddress)
        //{
        //    password = Security.getMD5ByStr(password);
        //    Model.UserInfo userInfo = DAL.BackgroundUserInfoDal.GetInfoByUserNameAndPwd(UseName, password);
        //    if (userInfo != null)
        //    {
        ...
        public static Model.UserInfo GetLoginUserInfo(long userId)
        {
        ...
        public static List<Model.UserInfo> GetAllUserInfoList(int pageSize, int currentPage, int roleType, string keyWords, out int allCount)
        {
            allCount = 0;
            List<Model.UserInfo> userInfoList = DAL.BackgroundUserInfoDal.GetPageListByCondition(pageSize, currentPage, roleType, out allCount, keyWords);
        ...
        public static Model.UserInfo GetSingleUserInfo(long id)
        {
        ...
        /// 比较新旧用户信息，返回修改的字段内容
        /// </summary>
        public static string GetDiffContent(UserInfo newUserInfo, UserInfo oldUserInfo)
        {
